...
Congratulations! You have just programmed a bootloader and it only gets better from here. In the next section we will be going over a second stage bootloader, kernel, drivers, video graphics, and more. If you want to mess around a bit more I've set up the print function to only allow one thing to print. If you try and print something else it'll overwrite what you had before. You can try and change this or play around with other interrupts. You can create an entire operating system with real mode and BIOS but we won't be doing that.
